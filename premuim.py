...
def handle_sol_trending_callbacks(call):
    ...
    print(f"SOL trending callback: {call.data}")
    ...
    if call.data.startswith("sol_") and "_" in call.data and call.data != "sol_back" and call.data != "sol_mainmenu":
        ...
        if len(parts) >= 3:
            ...
            bot.send_message(
                chat_id,
                f"""
📄 *Enter Token Name or Contract Address (CA)*

You selected `{price_display}`.
...
""",
            ...
            print(f"Set ca_retry_waiting for chat_id: {chat_id}")
